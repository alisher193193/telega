...
from app.bot.handlers import start, directories, people
...
BACK_TEXTS = {"Назад", "⬅ Назад", "◀️ Назад"}
CANCEL_TEXTS = {"❌ Отменить", "Отменить", "Отмена", "✖ Отменить", "✖ Отмена"}
...
async def section(message: Message, state: FSMContext, system_user: User):
    try:
        if message.text == "👥 Люди":
            await people.open_section(message, state, system_user)
...
async def control(message: Message, state: FSMContext, system_user: User):
    ...
    if message.text in BACK_TEXTS and current and current.startswith("PeopleStates:"):
        try:
            await people.back(message, state, system_user)
        except AppError as exc:
            await message.answer(str(exc), parse_mode=None)
    elif message.text in BACK_TEXTS and current and current.startswith("DirectoryStates:"):
        try:
            async with session_factory() as session:
                await require_permission(session, system_user.id, "projects.view")
